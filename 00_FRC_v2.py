...
def profit_goal(total_costs):
    ...
    while not valid:
        ...
        if profit_type == "unknown" and amount >= 100:
            dollar_type = check_string(f"Do you mean ${amount:.2f}. ie {amount:.2f} dollars? , y / n:")
            ...
            percent_type = check_string(f"Do you mean {amount}%? , y / n: ")
            if percent_type == "yes":
                profit_type = "%"
            else:
                profit_type = "$"
...
round_to = num_check("Round To: ", int)
...
to_write = [
    main_heading_string, quantity_string, "\n",
